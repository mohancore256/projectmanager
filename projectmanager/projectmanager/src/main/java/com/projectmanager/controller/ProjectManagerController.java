...
public class ProjectManagerController {
	...
	public ResponseEntity<?> getAllTask() {
		...
		if (null != taskList) {
			...
			return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(taskList);
		}
	}
	...
	@RequestMapping(value="projectManager/task/sort", method=RequestMethod.GET)
	public ResponseEntity<?> sortTask(@PathParam("columnName") String columnName){
		logger.info("sorting a project");
		List<Task> sortedTaskList = taskService.sortTask(columnName);
		if(null != sortedTaskList) {
			return ResponseEntity.ok(sortedTaskList);
		}else {
		    return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(sortedTaskList);	
		}
	}
}
